...
export default function Home() {
  ...
  // Handler to Toggle Complete / Pending Status
  ...
  // Handler to Save Edits made to a Todo
    ...
    <main className="max-w-2xl mx-auto px-4 py-12 min-h-screen transition-colors duration-200">
      ...
      <header className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200 dark:border-gray-800">
        ...
        <button
          onClick={toggleTheme}
          type="button"
          className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm hover:scale-105 transition-transform font-medium"
          ...
          <div className="flex border-t border-gray-100 dark:border-gray-700 pt-3 gap-1">
            {(['all', 'pending', 'completed'] as const).map((type) => (
              <button
                ...
                className={`flex-1 text-xs py-1.5 font-semibold rounded-md capitalize transition-colors ${
                  filter === type
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 px-4 border-2 border-dashed rounded-xl border-gray-200 dark:border-gray-800">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-xs mx-auto">
                Try modifying your current filter setting or search query to find entries.
              </p>
            </div>
          )}
        </div>
